...
# 加载和预处理数据
dataset_df = pd.read_csv('assets/dataset/waimai_10k.csv')
...
x_train, x_test, train_labels, test_labels = train_test_split(
    ...
    random_state=42    # 随机种子(为确保结果可以复现)
)
#因为数据集过大，这里仅采用20%的训练集和测试集进行训练和测试，以便快速验证模型效果
train_sample_size = int(len(x_train) * 0.2)
