...
class CharGRUDataset(Dataset):
    ...
    def __getitem__(self, idx):
        text = self.texts[idx]
        # 将字符转换为索引，超过max_len的部分截断，不足的部分补0
...
class GRUClassifier(nn.Module):
    ...
    def forward(self, x):
        embedded = self.embedding(x)  # 嵌入层：[batch_size, max_len] -> [batch_size, max_len, embedding_dim]
        # GRU输出：output为所有时间步的隐藏状态，hidden为最后一个时间步的隐藏状态
        gru_out, hidden = self.gru(embedded)  # GRU只有隐藏状态，没有LSTM的细胞状态
        # 取最后一个时间步的隐藏状态用于分类
        out = self.fc(hidden.squeeze(0))  # squeeze(0)移除批次维度中的冗余维度
        return out
...
def classify_text_gru(text, model, char_to_index, max_len, index_to_label):
    ...
    input_tensor = torch.tensor(indices, dtype=torch.long).unsqueeze(0)  # 增加批次维度
