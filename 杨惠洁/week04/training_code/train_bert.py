...
# 加载和预处理数据
dataset_df = pd.read_csv('assets/dataset/waimai_10k.csv', sep=',', header=0)

# 初始化 LabelEncoder，用于将文本标签转换为数字标签
lbl = LabelEncoder()
# 拟合数据并转换前500个标签，得到数字标签
texts = list(dataset_df['review'].values[:500])    # 文本内容是'review'列
labels = list(dataset_df['label'].values[:500])    # 标签是'label'列
...
# 从预训练模型加载分词器和模型
tokenizer = BertTokenizer.from_pretrained('./assets/models/dir')
model = BertForSequenceClassification.from_pretrained(
    './assets/models/dir', num_labels=12 # 判别式模型，分多少类
)
