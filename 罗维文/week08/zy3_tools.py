...
class ExtractionAgent:
    ...
    def call(self, user_prompt, response_model):
        ...
        try:
            ...
            # return None
            raise Exception(f"{e}\n'模型响应结果'{response.choices[0].message}")
...
def model_for_tools(request_text: Union[str, List[str]]) -> List[Dict[str, Union[str, Dict[str, List[str]]]]]:
    ...
    for text in texts:
        ...
        try:
            response = ExtractionAgent(model_name=model).call(text, IntentDomainNerTask)
            print(response)

            # 解析 LLM 输出
            result = {"domain": response.domain, "intent": response.intent, "slots": response.slots}
