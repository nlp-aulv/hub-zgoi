...
你作为一名专业内容审核员，你需要结合用户的提问和生成的回答，判断回答是否满足了提问的要求。如果满足，则直接回满足，不需要有其他输出。如果不满足，则输出还需要分析的子问题，只输出子问题列表（每行一个，无前缀）。
"""
...
async def process_section(section, i):
    ...
    **原始网页内容 (请基于此内容撰写):**
    ...
    try:
        ...
        # 3.4. 章节评估 (调用 Reflection Agent)

        print(f"-> 正在评估章节内容质量...")
        evaluation_prompt = f"""
                    提问: {section_query}
                    回答: {section_draft.final_output}
                    """
        evaluation_result = await Runner.run(
            ReflectionAgent,
            evaluation_prompt,
        )
        evaluation_output = evaluation_result.final_output.strip()
        print(f"-> 章节评估结果: {evaluation_output}")
        if evaluation_output == '满足':
            print(f"-> 章节 {section_title} 满足要求，生成完成")
        else:
            # 获取需要进一步研究的子问题
            sub_questions = [q.strip() for q in evaluation_output.split("\n") if q.strip]
            print(f"-> 需要进一步研究的子问题: {sub_questions}")
            # 对子问题进行额外检索
            additional_research = []
            for sub_q in sub_questions[:2]:  # 最多检索两个
                print(f"-> 正在检索子问题: {sub_q}")
                sub_search_results = await async_search_jina(sub_q)
                try:
                    sub_results = json.loads(sub_search_results)
                    sub_urls_to_crawl = [res['url'] for res in sub_results if res.get('url')][:2]  # top2相关的网页的url
                    for sub_url in sub_urls_to_crawl:
                        sub_content = await async_crawl_jina(sub_url)  # 读取最相关网页的内容
                        additional_research.append(
                            f"--- 子问题: {sub_q} ---\n--- URL: {sub_url} ---\n{sub_content[:2000]}...\n")
                except Exception as e:
                    print(f"-> 子问题检索失败: {e}")
            additional_materials = "\n\n".join(additional_research)
            draft_prompt = f"""
            **章节主题:** {section_title}
            ...
            **原始网页内容 (请基于此内容撰写):**
            ...
            {"**补充研究材料:**\n" + additional_materials if additional_materials else ""}
            ...
            drafted_sections = section_draft.final_output
            print(f"-> 章节内容生成完成")
...
"""
    ...
    # 改进点：前面的章节其实会影响后面的章节
    # 每个章节依次执行
    process_sections = [process_section(section, i) for i, section in enumerate(sections)]
    # 改进并发执行
    drafted_sections = await asyncio.gather(*process_sections, return_exceptions=True)
