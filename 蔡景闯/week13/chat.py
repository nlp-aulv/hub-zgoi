...
async def chat(user_name:str, session_id: Optional[str], task: Optional[str], content: str, tools: List[str] = []):
    ...
    if not tools or len(tools) == 0:
        chat_agent = Agent(
            name='闲聊智能体',
            handoff_description="闲聊智能体",
            instructions="你是一个和用户闲聊的智能体。",
        ...
        triage_agent = Agent(
            name="分类智能体",
            instructions="你根据用户的问题确定使用闲聊智能体还是股票智能体",
    ...
    # 需要调用mcp 服务进行回答
        ...
        async with mcp_server:
            ...
            triage_agent = Agent(
                name="分类专家",
                instructions="你是一个分类专家，你的任务是每次用户提问时，分析问题的类型，并引导至合适的专家",
                # mcp_servers=[mcp_server],
                handoffs=[chat_agent, stock_agent],
                model=OpenAIChatCompletionsModel(
                    model=os.environ["OPENAI_MODEL"],
