...
class GRUClassifier(nn.Module):
    ...
    def forward(self, x):
        embedded = self.embedding(x)
        # GRU 返回 gru_out 和 hidden_state，没有 cell_state
        gru_out, hidden_state = self.gru(embedded)
        # hidden_state 的形状为 (num_layers * num_directions, batch_size, hidden_dim)
        out = self.fc(hidden_state.squeeze(0))  # squeeze(0) 去掉第0维（层数），假设 num_layers=1
        return out
