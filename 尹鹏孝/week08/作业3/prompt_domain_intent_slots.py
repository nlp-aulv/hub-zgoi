        ...
        if completion.choices and len(completion.choices) > 0:
            ...
            if json_match:
                ...
                try:
                    ...
                    print(f"JSON解析失败: {e}")
                    # 如果解析失败，返回原始字符串
                    result_content = {"raw_response": result_content, "error": "JSON解析失败"}
            else:
                print("\n直接输出:")
                print(result_content)
                # 如果没有 JSON 标记，尝试直接解析整个响应
                try:
                    result_content = json.loads(result_content)
                except json.JSONDecodeError:
                    # 如果也不是有效的 JSON，返回原始文本
                    result_content = {"raw_response": result_content}

        else:
            print(1)
            print("未收到有效回复")
            result_content = {"error": "未收到有效回复"}

    except Exception as e:
        print(2)
        result_content = {"error": f"API调用失败: {str(e)}"}
